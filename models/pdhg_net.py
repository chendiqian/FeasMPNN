# https://github.com/NetSysOpt/PDHG-Net/blob/main/src_iplb/models.py
import torch
...
class BipartiteGraphConvolution(nn.Module):
    def __init__(self, emb_size, activation, initializer, right_to_left=False, **kwargs):
        ...
        self.temp = nn.Parameter(torch.Tensor(2))
    ...
    def __init__(self, emb_size, nConsF, nVarF, num_layers):
        ...
        for _ in range(num_layers):
            self.conv_v_to_c.append(
                BipartiteGraphConvolution(emb_size, self.activation, None, right_to_left=True, ))
            self.conv_c_to_v.append(BipartiteGraphConvolution(emb_size, self.activation, None))
        ...
        self.output_module1 = nn.Sequential(
            nn.Linear(in_features=emb_size, out_features=emb_size, bias=True),
            nn.ReLU(),
