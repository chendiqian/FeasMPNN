...
@hydra.main(version_base=None, config_path='./config', config_name="run_fix_step_ipm")
def main(args: DictConfig):
    ...
    for run in range(args.runs):
        ...
        trainer = FixStepIPMTrainer(args.losstype, args.microbatch)

        pbar = tqdm(range(args.epoch))
        for epoch in pbar:
            train_loss = trainer.train(BackgroundGenerator(train_loader, device, 2), model, optimizer)
            stats_dict = {'train_loss': train_loss,
                          'lr': scheduler.optimizer.param_groups[0]["lr"]}
            if epoch % args.eval_every == 0:
                val_obj_gap, val_violation = trainer.eval(BackgroundGenerator(val_loader, device, 2), model)
                ...
                stats_dict['val_violation'] = val_violation
        ...
        test_obj_gap = trainer.eval(BackgroundGenerator(test_loader, device, 4), model)
